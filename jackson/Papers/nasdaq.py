...
mlds_err = sio.loadmat('F:\\repos\multi_linear_research\jackson\Docs&Code\L-TVAR\data\err_nasdaq.mat')
mlds_error = pd.DataFrame(np.transpose(mlds_err['err_dct']), index=dct_error.index, columns=["dct-MLDS"])
df = pd.concat([mlds_error, lstm_error, dct_error, dcti_error], axis=1)
ax = df.plot()
ax.set_xlabel("Days")
ax.set_ylabel("Error")
plt.show()